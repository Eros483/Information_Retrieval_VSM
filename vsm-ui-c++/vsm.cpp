    ...
    , ui(new Ui::vsm)
{
    ...
    // Create horizontal layout to hold bar + button
    QHBoxLayout *corpusLayout = new QHBoxLayout();
    corpusLayout->addWidget(corpusBar);
    corpusLayout->addWidget(browseButton);
    layout->addLayout(corpusLayout);

    connect(browseButton, &QPushButton::clicked, this, [=](){
        QString folder = QFileDialog::getExistingDirectory(this, "Select Corpus Folder", QDir::homePath());
        if (!folder.isEmpty()) {
    ...
    // Initial state: show corpus bar and browse button
    corpusBar->show();
    corpusLabel->show();
    browseButton->show();
    corpusBar->setFocus();
